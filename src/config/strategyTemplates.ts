 ...
 * 获取策略模板
 */
export function getStrategyTemplate(templateId: string): StrategyTemplate | undefined {
  return STRATEGY_TEMPLATES.find((template) => template.id === templateId);
}
...
export function getTemplatesByPlatform(
  platform: 'pump.fun' | 'four.meme' | 'raydium' | 'uniswap' | 'pancakeswap'
): StrategyTemplate[] {
  return STRATEGY_TEMPLATES.filter(
    (template) => template.platform === 'all' || template.platform === platform
  );
}
 ...
 * 根据风险等级获取策略模板
 */
export function getTemplatesByRiskLevel(riskLevel: 'low' | 'medium' | 'high'): StrategyTemplate[] {
 ...
 * 根据类别获取策略模板
 */
export function getTemplatesByCategory(category: 'conservative' | 'moderate' | 'aggressive'): StrategyTemplate[] {
 ...
 * 应用策略模板
 */
export function applyStrategyTemplate(template: StrategyTemplate): {
  name: string;
  strategyType: string;
  platform: string;
  params: any;
  ...
  return {
    ...
    platform: template.platform === 'all' ? 'pump.fun' : template.platform,
